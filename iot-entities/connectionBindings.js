...
var connectionBindings = sequelize.define('connectionBindings', {
    id: {
        type: DataTypes.UUID,
        primaryKey: true,
...
exports.update = function (deviceId, server, connectingStatus, resultCallback) {
    ...
    if (connectingStatus === true) {
        record.lastConnectedAt = new Date().getTime();
        filters.fields.push('lastConnectedAt');
    }

    return connectionBindings.update(record, filters)
        .then(result => {
            if (!result) {
                ...
                record.id = uuid();
                record.lastConnectedAt = new Date().getTime();
                return connectionBindings.create(record);
            } else {
                return new Promise(resolve => {
                    resolve(result.length < 2 ? result[0] : result[1]);
                });
            }
        })
        .then(result => {
